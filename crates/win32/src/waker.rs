...
//! call [`Waker::wake`]. Here that posts [`Msg::Wake`] through win32ui's
...
/// Wakes the Win32 frontend by posting a message to the window.
pub struct Win32Waker(Proxy<Msg>);

impl Win32Waker {
    /// Wraps the window's proxy, obtained from [`win32ui::Ui::proxy`].
    #[must_use]
