//! Windows System Media Transport Controls (SMTC, #320).
//!
...
//! single implementation of the transport logic instead of a second, parallel
//! one.
//!
...
pub struct Smtc {
    ...
    /// tick by [`Smtc::sync`]. The handler runs on a WinRT callback thread,
    ...
    /// a second instead of every tick.
...
impl Smtc {
    ...
    /// Runs once per tick: drains the OS's transport events into queued
    ...
    fn drain_events(&mut self, player: &dyn PlayerApi, state: &mut AppState) {
        ...
        while let Ok(event) = events.try_recv() {
            if let Some(command) = transport_command(event, status, seek_supported) {
                state.push(command);
            }
        }
    }
    ...
    fn publish(&mut self, player: &dyn PlayerApi, library: &dyn LibraryDataSource) {
        ...
        // A track with no known length publishes no progress, so the OS
        // overlay can't offer a scrubber for something it can't measure.
        let progress = player
            .duration()
            .is_some()
...
///
/// `Play`/`Pause` can arrive while already in that state; toggling then would
/// do the opposite of what the button says, so they are gated on `status`.
/// `SetPosition` is only forwarded when `seek_supported`, otherwise a SID tune
/// would ignore the drag while the OS overlay kept showing the requested
/// position.
fn transport_command(
    ...
    match event {
        MediaControlEvent::Play => {
            matches!(status, PlaybackStatus::Paused | PlaybackStatus::Stopped)
                .then_some(Command::PlayerPlayPause)
        }
        MediaControlEvent::Pause => {
            (status == PlaybackStatus::Playing).then_some(Command::PlayerPlayPause)
        }
        ...
        MediaControlEvent::SetPosition(position) if seek_supported => {
...
/// blocks; [`CoverLookup::drain`] collects finished lookups once per tick.
#[derive(Default)]
struct CoverLookup {
