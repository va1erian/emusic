...
pub fn show(ui: &mut egui::Ui) {
    ...
    ui.label(
        egui::RichText::new(
            "Choose which audio file types open with emusic, then Register. \
             Windows 10/11 may still ask you to confirm the change in Settings.",
        )
        .weak(),
    );
    ui.add_space(8.0);
    ...
    let error_id = ui.id().with("assoc_error");
    ui.horizontal(|ui| {
        if ui.button("Register").clicked() {
            let result = register_selected(&manager, &selected);
            store_result(ui, error_id, result);
        }
        if ui.button("Unregister all").clicked() {
            let result = manager.unregister().map_err(|e| e.to_string());
            store_result(ui, error_id, result);
        }
        if ui.button("Open Windows Default Apps...").clicked() {
            let result = open_default_apps_settings(APP_NAME).map_err(|e| e.to_string());
            store_result(ui, error_id, result);
        }
    });

    if let Some(error) = ui.ctx().data(|d| d.get_temp::<String>(error_id)) {
        ui.colored_label(ui.visuals().error_fg_color, error);
    }
}

fn register_selected(manager: &AssocManager, selected: &[bool]) -> Result<(), String> {
...
/// frame can render it, clearing any previous error on success.
fn store_result(ui: &egui::Ui, id: egui::Id, result: Result<(), String>) {
    match result {
        Ok(()) => ui.ctx().data_mut(|d| d.remove::<String>(id)),
        Err(err) => {
            tracing::warn!(%err, "file association action failed");
            ui.ctx().data_mut(|d| d.insert_temp(id, err));
        }
    }
}
